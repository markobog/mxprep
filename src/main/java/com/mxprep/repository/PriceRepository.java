...
public interface PriceRepository extends JpaRepository<Price, Integer> {
    ...
    @Query("SELECT p.company as company, p.close as close, p.open as open, p.low as low, p.high as high, " +
    ...
    @Query("SELECT p.company as company, p.close as close, p.open as open, p.low as low, p.high as high, " +
            ...
            "FROM Price p " +
            "WHERE p.date = :date " +
            "ORDER BY normalized DESC LIMIT 1")
    List<PriceNormalizedStatistics> getPriceNormalizedMaxForDate(@Param("date") LocalDate date);
